...
def compute_metrics(y_true: List[int], y_prob: np.ndarray) -> Dict:
    """
    y_prob: shape [N, 2] class probabilities; positive class assumed index=1
    """
    ...
    try:
        ...
        roc_auc = float("nan")
    try:
        pr_auc = average_precision_score(y_true, pos_scores)
    except ValueError:
        pr_auc = float("nan")
...
def plot_roc(y_true: np.ndarray, y_prob1: np.ndarray, out_path: str, auc_val: float | None = None):
...
def plot_pr(y_true: np.ndarray, y_prob1: np.ndarray, out_path: str, ap_val: float | None = None):
    ...
    # Baseline at positive class prevalence
...
def plot_confusion(y_true: np.ndarray, y_pred: np.ndarray, out_path: str, normalize: bool = False):
...
def plot_calibration(y_true: np.ndarray, y_prob1: np.ndarray, out_path: str, n_bins: int = 15):
...
def plot_score_hist(y_prob1: np.ndarray, out_path: str):
    plt.figure()
    plt.hist(y_prob1, bins=30, alpha=0.9)
    plt.xlabel("P(class=1)")
    plt.ylabel("Count")
    plt.title("Score Histogram (Positive Class Probability)")
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
...
def evaluate_and_plot(y_true: List[int], y_prob: np.ndarray, out_dir: str, prefix: str = "val") -> Dict:
    ...
    # Plots (now annotated with AUC / AP)
    try:
        plot_roc(y_true, pos, os.path.join(out_dir, f"{prefix}_roc.png"), auc_val=metrics.get("roc_auc"))
    except ValueError:
        pass
    try:
        plot_pr(y_true, pos, os.path.join(out_dir, f"{prefix}_pr.png"), ap_val=metrics.get("pr_auc"))
    except ValueError:
        pass
    ...
    try:
        plot_calibration(y_true, pos, os.path.join(out_dir, f"{prefix}_calibration.png"))
    except Exception:
        pass
